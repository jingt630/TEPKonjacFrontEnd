...
export default class MediaManagementConcept {
   ...
   */
  async delete({ userId, mediaId }: { userId: ID; mediaId: ID }): Promise<Empty | {error: string}> {
   ...
   */
  async _getMediaFile({ userId, mediaId }: { userId: ID; mediaId: ID }): Promise<MediaFile[]> {
   ...
   */
  async _listMediaFiles({ userId, filePath }: { userId: ID; filePath: string }): Promise<MediaFile[]> {
    return await this.mediaFiles
   ...
   */
  async _listFolders({ userId, filePath }: { userId: ID; filePath: string }): Promise<Folder[]> {
